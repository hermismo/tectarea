<div class="tab-content">
	...
	<form action="<?= base_url()."negocio/activarServicio"."/".$id."/".$idPersona?>" method="post"> 
					...
					<?php
                       for ($i=0; $i < count($antiguedad); $i++) { 
                           ...
                           for ($k=1; $k < 7; $k++) { 
                            ...
                            echo '<input type="text" name="precio[]" value="'.$antiguedad[$i][$k]['precio_antiguedad'].'" size="5" maxlength="8" onkeyPress="return validarNumero(event)" required> <br>';
		...
		<div id="sombra">
			<label>
			  <input type="radio" name="visado_s" value="Si" class="visado_si" required> Si
			  <div id="">
		    ...
		    <br>
		    <label>
		      <input type="radio" name="visado_s" value="No" class="visado_no" required> No
		    </label>
		</div>
