<div class="tab-content">
	<strong><h3>Tus precios y servicios | Informe de Evaluación de Edificios</h3> </strong><br>
	...
	<form action="<?= base_url()."negocio/activarServicio"."/".$id."/".$idPersona?>" method="post"> 
		<input type="hidden" value="" id='id_IEE' name="id_servicio">
							<div id="sombra">
			...
			<table class="table" id="tabla_c">			
					...
					<?php
                       for ($i=0; $i < count($antiguedad); $i++) { 
                           ...
                           for ($k=1; $k < 7; $k++) { 
                            echo '<td>';
                            echo "<input type='hidden' name='superficie[]' value='".$antiguedad[$i][$k]['id_antiguedad2']."'>";
                            echo '<input type="text" name="precio[]" value="'.$antiguedad[$i][$k]['precio_antiguedad2'].'" size="5" maxlength="8" onkeyPress="return validarNumero(event)" required> <br>';
                             echo '<font color="red">'.$antiguedad[$i][$k]['precio_antiguedad2'].'</font>';
                              echo '</td>';
		...
		<div id="sombra">
			<label>
			  <input type="radio" name="visado_s" value="Si" id="visado_si" required> Si
			  <div id="">
			    ...
			    </label><small>*A tus precios añadiremos el precio del visado</small>
		    </div>
		    <br>
		    <label>
		      <input type="radio" name="visado_s" value="No" id="visado_no" required> No
		    </label>
		</div>
	...
	<button type="submit" class="btn btn-success btn-lg btn-block" name="boton_s" value="IEE">Guardar / Activar</button>   
    </form>
