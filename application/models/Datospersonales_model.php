...
class Datospersonales_model extends CI_Model {
    ...
    public function verificarImagen($imagen,$idUsuario){
        ...
        $query=$this->db->where($condicion)->get('tecnico');
    ...
    public function insertarImagen($id_tecnico,$idTipoImagen,$imagen){
        $sql=$this->db->query('SELECT * FROM imagen where id_tecnico='.$id_tecnico.' and id_tipo_imagen='.$idTipoImagen.' ');

        if ($sql->num_rows() > 0) {
           $this->db->query('UPDATE imagen SET nombre_imagen="'.$imagen.'" WHERE id_tecnico='.$id_tecnico.' and id_tipo_imagen='.$idTipoImagen.' ');
        }
        else{
            $datos=array(
                'id_tecnico'=>$id_tecnico,
