    ...
    if(archivo.length==0){
        ...
        $.ajax({
            ...
            success: function(data){
                if(data==0){
                    message = $("<span class=' alert alert-danger alert-dismissable'>La imagen cargada no es válida, por favor verifique e intente nuevamente.</span>");
                    showMessage(message);
                }
                else if(data==1){
                    message = $("<span class=' alert alert-danger alert-dismissable'>Ha ocurrido un error al guardar la imagen. Por favor contacte al administrador del sistema.</span>");
                    showMessage(message);
                }
                else{ 
                    message = $("<span class=' alert alert-success alert-dismissable'>La imagen se ha cargado con éxito.</span>");
                    showMessage(message); 
