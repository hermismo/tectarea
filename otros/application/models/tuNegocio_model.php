...
class Tunegocio_model extends CI_Model
{
	...
	function activarServicio($datos){
		#falta ID DEL SERVICO
		$id_servicio=$datos['id_servicio']; #Capturar
        ...
        if ($datos['visado'] == "null" || $datos['visado'] == 'No') {
        		$this->db->query('insert into visado_servicio (monto) values (0)');
        		#obtener el id del monto insertado
				$sql_id=$this->db->query('select max(id_visado_servicio) as id from visado_servicio limit 1');
				foreach ($sql_id->result() as $result2){$id_vs=$result2->id;}        				
        		$this->db->query('insert into  visado_tecnico (id_condicion_visado, id_visado_servicio) values (2, '.$id_vs.')');
        		#obtener el id del visado para la tabla servicio _ tecnico
				$sql_idt=$this->db->query('SELECT id_visado_servicio as id, monto from visado_servicio where visado_servicio.id_visado_servicio=(select MAX(id_visado_servicio) from visado_servicio) limit 1');
				foreach ($sql_idt->result() as $result3){$id_vt=$result3->id; $monto_v=$result3->monto;}      
        	}else
        	{
        		$this->db->query('insert into visado_servicio (monto) values ('.$datos['monto_visado'].')');
        		#obtener el id del monto insertado
				$sql_id=$this->db->query('select max(id_visado_servicio) as id from visado_servicio limit 1');
				foreach ($sql_id->result() as $result2){$id_vs=$result2->id;}        				
        		$this->db->query('insert into  visado_tecnico (id_condicion_visado, id_visado_servicio) values (1, '.$id_vs.')');
        		#obtener el id del visado para la tabla servicio _ tecnico
				$sql_idt=$this->db->query('SELECT id_visado_servicio as id, monto from visado_servicio where visado_servicio.id_visado_servicio=(select MAX(id_visado_servicio) from visado_servicio) limit 1');
				foreach ($sql_idt->result() as $result3){$id_vt=$result3->id; $monto_v=$result3->monto;} 
	     ...
	     if ($datos['servicio'] == "ITE" || $datos['servicio'] == "IEE"){
	     	...
	     	for($k=0; $k< count($datos['tipologia']); $k++){ 
		    	...
		    	for($i=0; $i<count($datos['precio'])/count($datos['tipologia']); $i++)
			          {echo $datos['precio'][$cont]."<br>";
			          $total=$datos['precio'][$cont]+$monto_v;	
			         $this->db->query('INSERT INTO superficie_antiguedad(id_superficie, id_antiguedad, id_servicio_tecnico, precio) VALUES('.$datos['tipologia'][$k].','.$datos['superficie'][$i+1].','.$id_servicio_tecnico.','.$total.')');
	...
	function datos_servicio($datos){
		...
		$sql2=$this->db->query('select st.id_servicio_tecnico,st.id_servicio, st.descripcion_servicio, st.id_estatus, st.visado from servicio_tecnico as st WHERE st.id_tecnico='.$datos['id_tecnico'].' and st.id_servicio='.$datos['id_servicio'].' limit 1');
		$sql=$this->db->query("select id_tipologia,tipo_tipologia from tipologia order by id_tipologia");
		$i=0;
		foreach ($sql2->result() as  $fila_tecnico) {#Extraer el id del servicio_tecnico para hacer la consulta correspondiente al servicio y al tecnico
			...
			$datos['estatus']=$fila_tecnico->id_estatus;
